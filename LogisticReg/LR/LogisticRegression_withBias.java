...
public class LogisticRegression_withBias {
        ...
        public void train(List<LRInstance> instances) {
            for (int n = 0; n < ITERATIONS; n++) {
                ...
                for (int i=0; i < instances.size(); i++) {
                    ...
                    double[] feats = instances.get(i).x; // get the appropriate features
                    double pred = this.probPred1(feats); // get the prediction
                    double sum = 0.0; //inital sum
                    for(int w = 0; w < weights.length; w++){ // loop through the weight vectors
                          weights[w] = weights[w] + (this.rate*feats[w]) *(instances.get(i).label  - pred); // apply the gradiant ascent formula 
                          sum += (weights[w] * feats[w]);
                    }
                    // TODO: Compute the log-likelihood of the data here. Remember to take logs when necessary
                    lik += instances.get(i).label * sum - Math.log(1 + Math.exp(sum)); // apply equation given the lecture slides
                }
        ...
        public static class LRInstance {
            ...
            public LRInstance(int label, double[] x) {
                this.label = label;
                this.x = new double[x.length + 1]; // get the instance
                for(int i = 0; i < x.length; i++){
                    this.x[i] = x[i]; // intialize the instance object
                }
                this.x[x.length] = 1;
            }
        }
